...
print('The weighted sum of the inputs at the node in the output layer is {}'.format(np.around(z_12, decimals=4)))
...
print('NEW NETWORK = small_network >>>>>>>>>>>>>>>>>>>>>>>>>>>')

small_network = initialize_network(5, 3,[3,2,3],1)

print('compute_weighted_sum >>>>>>>>>>>>>>>>>>>>>>>>>>>')
...
inputs = np.around(np.random.uniform(size=5), decimals=2)
predictions = forward_propagate(new_net, inputs)
print('The predicted values by the new_net for the given input are {}'.format(predictions))
